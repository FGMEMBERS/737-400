...
setlistener("/autopilot/route-manager/route/wp/id", func {
 #wpid = cmdarg().getValue();
 wpid = getprop("/autopilot/route-manager/route/wp/id");
...
setlistener("/sim/signals/fdm-initialized", func {
    ...
    setprop("autopilot/settings/target-speed-kt",110);
	 setprop("autopilot/settings/target-speed-mach",0.74);
...
#    setprop("/instrumentation/flightdirector/TAS-kt", 0.0);
    ...
    setprop("/instrumentation/flightdirector/mach-mode",0); #if 0 target-speed by IAS, if 1 target-speed by mach
...
vnav = getprop("/instrumentation/flightdirector/fd-on");
if(ap_on==1){
...
# alt_selest = alt_select + getprop("/environment/ground-elevation-m")*3.281;
	gsdis = getprop("/instrumentation/nav/gs-distance");
	if(gsdis == nil) { gsdis = 0.0; }
	alt_select = (gsdis*FD_TAN3DEG) + (getprop("/environment/ground-elevation-m"))*3.281;
}
...
#????????????????????????????????????????????????????????????????????????????
#
# Funciones para pasar de IAS a mach y viceversa                            ?
#
#????????????????????????????????????????????????????????????????????????????
ias_to_mach = func {
	alt_ma = getprop("/instrumentation/altimeter/indicated-altitude-ft");
	alt_ma = int(alt_ma / 1000.0);
	v_sonido = 29.06 * (math.sqrt(518.7 - (3.57 * alt_ma)));	
	kias = getprop("velocities/airspeed-kt");	
	ktas = kias + ((kias * 0.02)*alt_ma);	
	mach_ma = ktas / v_sonido;	
	print("Mach calculado - " ~ mach_ma);
}
...
mach_to_ias = func {	
	...
	v_sonido = 29.06 * (math.sqrt(518.7 - (3.57 * alt_ma)));	
	ktas = 0.74 * v_sonido;	
	kias = ktas / (1 + (0.02 * alt_ma));	
	return kias;
	#return int(0.74 * v_sonido); #se asciende a mach 0.74 mÃ¡ximo
}

#If descending -1, if ascending 1 level 0
descending = func (dCurrentFpm) {
	if(dCurrentFpm < -50.0) { return -1; }
	elsif (dCurrentFpm > 50.0) { return 1; }
	return 0;
}

var vnavIAS_flag = 0;
vnav_navig = func {
 	 mivnv = getprop("/autopilot/locks/vnav");
    mialt = getprop("/instrumentation/altimeter/indicated-altitude-ft");
    if(mialt == nil)
    {
		#print("ALTURA INDICADA SALE COMO NIL");
		mialt = 0;
    }
    if(mivnv != 0)
    {
		if(mialt < 10000.0)
		{
			setprop("/instrumentation/flightdirector/mach-mode", 0);		
		...
		elsif(mialt < 26000.0)
		{
			...
			fpmav = getprop("/instrumentation/vertical-speed-indicator/indicated-speed-fpm");
			if(fpmav == nil) { fpmav = 0.0; }
			if(fpmav < -100.0) { setprop("autopilot/settings/target-speed-kt",310); }
			else { setprop("autopilot/settings/target-speed-kt",315); }
		}
		else
		{
			fpmav = getprop("/instrumentation/vertical-speed-indicator/indicated-speed-fpm");
			if(descending(fpmav) == -1) #if descending
			{
				dCurrentSpeed = getprop("/instrumentation/airspeed-indicator/indicated-speed-kt");
				dTargetSpeed = getprop("autopilot/settings/target-speed-kt");
				if( vnavIAS_flag == 0)
				{	
					if( (dTargetSpeed - dCurrentSpeed) < 10.0)
					{
						setprop("/instrumentation/flightdirector/mach-mode", 0);
						setprop("/autopilot/locks/speed", "speed-with-throttle");
						vnavIAS_flag = 1;						
					...
					{
						setprop("autopilot/settings/target-speed-mach",0.69); #verificar este valor
						setprop("/autopilot/locks/speed", "mach-hold");
						setprop("/instrumentation/flightdirector/mach-mode", 1);
					}				
				}
			}
			elsif(descending(fpmav) == 1) #if climbing
			{
				setprop("autopilot/settings/target-speed-mach",0.69); #verificar este valor
				setprop("/autopilot/locks/speed", "mach-hold");
				if(vnavIAS_flag == 1) { vnavIAS_flag = 0;	}
			}
			else #level flight
			{
				setprop("/instrumentation/flightdirector/mach-mode", 1);	
			...
			#setprop("autopilot/settings/target-speed-kt",mach_to_ias());
		}
    }
}
