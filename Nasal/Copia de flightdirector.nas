...
#------------------------------------------------------------------
	...
	mialt = getprop("/position/altitude-ft");
    if(mivnv != 0.0)
    {
	if(mialt < 10000.0)
	{
	...
	{
		setprop("autopilot/settings/target-speed-kt",315);
	}
    }
#-------------------------------------------------------------------
    registerTimer();
...
#############################################################################
# Modificación por VaronRojo: Cuando esté activado VNAV pondrá mensajes en pantalla
# Si estás por debajo de 10.000 pies, ajusta la velocidad a 250kt.
# Si estás por encima de 10.000 pies, ajusta la velocidad a 315kt. 
#############################################################################
setlistener("/autopilot/locks/vnav", func {
	mivnav = getprop("/autopilot/locks/vnav");
	if(mivnav != 0.0)                #Si VNAV está activado
	{ 
		print("VNAV ACTIVADO"); 
		#current_alt = getprop("/instrumentation/altimer/indicated-altitude-ft");  #Se toma la altura actual
		current_alt = getprop("/position/altitude-ft");
		#print("Altura actual...");
		#print(current_alt);
		if( current_alt >= 10000.0)
		{
			setprop("autopilot/settings/target-speed-kt",305);
			#print("Velocidad ajustada a 315 nudos");
		}
		else
		{
			setprop("autopilot/settings/target-speed-kt",250);
			#print("Velocidad ajustada a 250 nudos");
		}
	}
});
